...
- [深入理解Linux内核-信号](https://wushifublog.com/2020/05/16/%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3Linux%E5%86%85%E6%A0%B8%E2%80%94%E2%80%94signals/)
	...
	- 用户未给出自定义信号处理函数时。每一个信号都有默认的处理方式，可能是：terminate、dump（终止并生成 core file）、ignore、stop、continue
		- 执行默认处理时，也是在用户空间，应该也会使用一个临时创建的用户空间栈
	- 可见，在执行信号处理函数之前，需要保存用户应用程序的执行上下文，以便后续恢复程序的正常执行
	- 可见，在执行完信号处理函数时，需要再次进入内核空间，进行应用程序执行流的恢复

## 基础知识
- 同一个类型的信号只会存在一个被挂起的情况，所以也就是在信号没有被响应前，重复发送多次的信号，进程也最终也只会响应一次
- 按照POSIX要求，信号应该被进程中的所有线程共享，不过每个线程有自己独有的 mask，用于配置 block 哪些信号
- *信号只会被其中一个线程响应，这个线程是所有没有屏蔽此信号的线程中的任意一个*

## 常见信号
忽略：进程收到信号，但是没有做任何处理
阻塞：进程不收信号
`SIGKILL`：*不可被捕获、忽略、阻塞*，取值为9
`SIGINT`：可以被捕获、忽略、阻塞，取值为2，由`ctrl+c`产生
`SIGTERM`：可以被捕获、忽略、阻塞，`kill`命令默认发这个信号
`SIGQUIT`)：类似SIGINT，由`ctrl+\`产生
`SIGSTOP`：*不可被忽略、捕获、阻塞*，停止程序
